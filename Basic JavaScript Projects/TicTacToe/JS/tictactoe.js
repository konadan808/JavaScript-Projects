...
function placeXOrO(squareNumber) {
    ...
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        if (activePlayer==="X") {
            ...
            select.style.backgroundImage='url("Images/X-Wing.png")';
            // Active player may only be "X" or "O" so, if not "X" it must be "O"
        } else {
            // If activePlayer is equal to "O", the o.png is placed in HTML.
            select.style.backgroundImage= 'url("Images/TIE-O.png")';
        }
        ...
        if(activePlayer==="O") {
            /// This function d9isables clicking for computer choice
            disableClick();
            // This function waits 1 second before computer places image and eneables click.
...
// This function parses the selectedSquares array to search for win conidtions.
...
function checkWinConditions() {
    ...
    // O 0, 1, 2 condition.
    if  (arrayInclues("0O", "1O", "2O")) {drawWinLine (50, 100, 558, 100) }
    // O 3, 4, 5 condition
    ...
    //      each win condition.
    function arrayInclues(squareA, squareB, squareC) {
